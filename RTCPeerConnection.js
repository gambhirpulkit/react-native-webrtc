...
export default class RTCPeerConnection extends EventTarget(PEER_CONNECTION_EVENTS) {
  ...
  addTransceiver(source: 'audio' |'video' | MediaStreamTrack, init) {
    // Modification by Emitrr.
    // The `init` object contains MediaStreamTracks which become frozen
    // after passing through the React Native method below.
    // This causes a crash on trying to mute the stream later.
    // For now, WebRTCModule seems to be reading only the `direction`
    // property from the `init` object. Should be vigilant to make
    // sure there are no errors because _init might be expected to
    // contain other properties from `init`.
    let _init = { direction: init.direction };
    ...
    return new Promise((resolve, reject) => {
      ...
      WebRTCModule.peerConnectionAddTransceiver(this._peerConnectionId, {...src, init: { ..._init } }, (successful, data) => {
        if (successful) {
